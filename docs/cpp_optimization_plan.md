...
- `smoke`：每类 1 个 `.mat` 文件，处理选中 `.mat` 文件内的全部样本，用于验证预计算、训练和评估流程是否能跑通。
- `debug`：每类 3-5 个 `.mat` 文件，处理选中 `.mat` 文件内的全部样本，用于比较 CPP 归一化、FAM 参数和模型结构。
- `full`：全量数据，只用于验证已经在 `debug` 阶段稳定有效的方案。

预计算脚本需要支持按类别均衡选择 `.mat` 文件，避免只使用现有 `--max-files` 造成类别不均衡。选中 `.mat` 文件后应处理该文件内所有可切分样本。建议增加：
